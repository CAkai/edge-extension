import { getLocalStorage } from "../../packages/localstorage";
...
export const userStorage: UserStorage = {
    ...storage,
    save: async (newUser) => {
        localStorage.setItem(import.meta.env.VITE_ICLOUD_STORAGE_KEY, newUser.icloud.access_token);
    ...
    load: async () => {
        ...
        if (!user.icloud.access_token) {
            ...
            const token: string = data?.[import.meta.env.VITE_ICLOUD_STORAGE_KEY] ?? "";
